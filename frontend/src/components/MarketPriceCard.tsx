...
import { MapPin, Building, Calendar, Navigation, Sparkles, TrendingUp } from 'lucide-react';
...
export const MarketPriceCard: React.FC<MarketPriceCardProps> = ({ record, isFarmerCrop }) => {
    ...
    >
        ...
        <div className="space-y-1.5 text-xs text-slate-600 mb-4">
          <div className="flex items-center justify-between gap-1">
            ...
            {record.distance_km !== undefined && record.distance_km !== null && (
              <span className="text-[10px] font-black text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0">
      ...
      <div className="bg-slate-50/90 border border-slate-200/80 rounded-xl p-3 space-y-2.5">
        ...
        <div className="pt-2 border-t border-slate-200/60 flex items-center justify-between text-[11px] text-slate-600 font-medium">
          <span>Traded Range:</span>
          <span className="font-bold text-slate-800">
            ₹{record.min_price?.toLocaleString()} – ₹{record.max_price?.toLocaleString()} <span className="text-[10px] font-normal text-slate-500">/ Qtl</span>
          </span>
        </div>
      </div>
    </div>
